...
const GiftingSection = () => {
  return (
    <section className="w-full py-20 flex flex-col items-center bg-gradient-to-br from-[#fff8f0] to-[#f7e9ff] relative overflow-hidden">
      ...
      <h2 className="text-4xl md:text-5xl font-extrabold text-[#bfa14a] mb-2 font-playfair z-10 tracking-wide">Jewelry Gifts That Sparkle</h2>
      <p className="text-lg text-gray-700 mb-8 max-w-xl text-center z-10 font-serif">Give the gift of luxury and elegance. Discover our exclusive jewelry picks for every special moment.</p>
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-10 w-full max-w-5xl mb-10 z-10">
        {gifts.map(gift => (
          <div key={gift.name} className="bg-gradient-to-br from-[#fffbe6] to-[#f7e9ff] rounded-3xl shadow-xl flex flex-col items-center p-8 group relative overflow-hidden transition-all duration-300 hover:shadow-2xl hover:-translate-y-2 hover:scale-105 border-2 border-[#e2c17c]">
            {/* Sparkle animation */}
            {gift.sparkle && (
